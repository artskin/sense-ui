...
function watchColorPicker(event) {
  document.styleSheets[1].addRule(':root','--primary:'+event.target.value);
...
function showThemes(){
  $_('#theme-layer').classList.toggle('show');
  let skinItems = $_('body').classList;
  if(window.frames['main']){
    skinItems = window.frames['main'].document.body.classList;
  }
  $_('#theme-layer').addEventListener('click',(e)=>{
    let theSkin = e.target;
    //console.log(skinItems.item);
    skinItems.remove('skin-ant','skin-vant','skin-cube')
    skinItems.add(theSkin.innerText)
    $_('#theme-layer').classList.remove('show');
  })
}
